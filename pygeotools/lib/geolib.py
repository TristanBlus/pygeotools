...
def mem_ds(res, extent, srs=None, dtype=gdal.GDT_Float32):
    ...
    if len(res)>1:
        ...
        res_x = res[0]
        res_y = res[1]
    dst_ns = int((extent[2] - extent[0])/res_x + 0.99)
    dst_nl = int((extent[3] - extent[1])/res_y + 0.99)
    m_ds = gdal.GetDriverByName('MEM').Create('', dst_ns, dst_nl, 1, dtype)
    m_gt = [extent[0], res_x, 0, extent[3], 0, -res_y]
...
def get_outline(ds, t_srs=None, scale=1.0, simplify=False, convex=False):
    ...
    if a.count() != 0:
        ...
        # x = np.concatenate((edges[1][::1], [edges[1][0]]))
        # y = np.concatenate((edges[0][::1], [edges[0][0]]))
        # x = np.concatenate((edges[1][::1], edges[1][::-1], [edges[1][0]]))
        y = np.concatenate((px[0][0][::1], px[1][0][::-1], [px[0][0][0]]))
        # y = np.concatenate((edges[0][::1], edges[1][::-1], [edges[0][0]]))
...
def get_res_stats(ds_list, t_srs=None):
    ...
    if len(res) > 1:
        min = [np.min([res[0][0],res[1][0]]),np.min([res[0][1],res[1][1]])]
        max = [np.max([res[0][0],res[1][0]]),np.max([res[0][1],res[1][1]])]
        mean = [np.mean([min[0],max[0]]),np.mean([min[1],max[1]])]
        med = [np.median([min[0],max[0]]),np.median([min[1],max[1]])]
    else:
...
def get_ds_srs(ds):
    ...
    # fix bugs in srs comparsion due to parameters missing
    if ds_srs.ExportToProj4() == wgs_srs.ExportToProj4():
        ds_srs = wgs_srs
    return ds_srs
...
def res_compare(r1, r2, precision=1E-3):
    ...
    #r2_f = '%0.6f' % r2
    if len(r1)>1:
        r1_f = [round_nearest(i, precision) for i in r1]
        r2_f = [round_nearest(i, precision) for i in r2]
    else:
